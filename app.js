...
const checkDivideByZero = (num1, num2) => {
    if (num2 <=0 )
...
document.addEventListener('DOMContentLoaded', ()=> {
    ...
    document.addEventListener('click', (event) => {
        ...
        //Return if the click is not on any of the button
        if (!event.target.matches('button')) {
            return;
        }

        //Parse the number values
        let btnClicked = parseInt(event.target.value);  
        //If the button clicked is not a number, get the value as such
        if(Number.isNaN(btnClicked)) 
            btnClicked = event.target.value;  
        
        //If AC button was clicked clear the result textbox and clear the number Array
        if (btnClicked === "clear") {
        ...
        if(!isOperatorClicked) {
            if (document.getElementById("result").innerHTML.trim() !== '0')
                document.getElementById("result").innerHTML += btnClicked; //event.target.value;
            else
                document.getElementById("result").innerHTML = btnClicked; //event.target.value;
        }
